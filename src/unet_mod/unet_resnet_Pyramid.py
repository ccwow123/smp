...
# 使用bridge='sppf'时，使用的是SPPF结构作为桥梁
class Unet_resnet_bridge(nn.Module):
    def __init__(self, input_channels=3,num_classes=2,depth=18,bridge='sppf'):
        super().__init__()
        if depth == 18:
            layers = [2, 2, 2, 2]
            block=BasicBlock
        ...
        if bridge == 'sppf':
            self.bridge = SPPF(nb_filter[4])
        elif bridge == 'sppcspc':
            self.bridge = SPPCSPC(nb_filter[4], nb_filter[4])
        elif bridge == 'rfb':
            self.bridge = BasicRFB(nb_filter[4], nb_filter[4])
        elif bridge == 'cbam':
            self.bridge = CBAM(nb_filter[4])
        else:
...
if __name__ == '__main__':
    ...
    net = Unet_resnet_bridge(num_classes=3, depth=18, bridge='sppf')
